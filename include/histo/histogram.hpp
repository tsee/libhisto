 ...
 */
class Histogram : public HistogramView {
 public:
  // Default constructor creates an empty/invalid handle
  constexpr Histogram() noexcept : HistogramView(nullptr) {}

  // Wrap an existing raw C pointer, taking ownership
  explicit constexpr Histogram(histo_t* handle) noexcept : HistogramView(handle) {}

  // Direct Constructor (Uniform binning)
  Histogram(uint32_t nbins, double min, double max, uint32_t flags = 0) noexcept
      : HistogramView(histo_create_uniform(nbins, min, max, flags)) {}
  ...
  // Destructor releases the C handle
  ~Histogram() noexcept {
    if (mut_handle()) {
      histo_destroy(mut_handle());
      handle_ = nullptr;
    }
  }

  // Move-only semantics (no implicit copying)
  Histogram(Histogram&& other) noexcept : HistogramView(other.handle_) {
  ...
  Status Fill(double x) noexcept {
    ...
    return histo_fill(mut_handle(), x);
  }

  Status Fill(double x, double weight) noexcept {
    if (!handle_) return Status(StatusCode::kInvalidArgument);
    return histo_fill_w(mut_handle(), x, weight);
  }

  Status Fill(Span<const double> samples) noexcept {
    if (!handle_) return Status(StatusCode::kInvalidArgument);
    if (samples.empty()) return OkStatus();
    return histo_fill_n(mut_handle(), samples.size(), samples.data(), nullptr);
  }
  ...
  Status Fill(Span<const double> samples, Span<const double> weights) noexcept {
    ...
    if (samples.empty()) return OkStatus();
    return histo_fill_n(mut_handle(), samples.size(), samples.data(), weights.data());
  }

  // Mutators & Operations
  Status Reset() noexcept {
    if (!handle_) return Status(StatusCode::kInvalidArgument);
    return histo_reset(mut_handle());
  }

  Status Scale(double factor) noexcept {
    if (!handle_) return Status(StatusCode::kInvalidArgument);
    return histo_scale(mut_handle(), factor);
  }

  Status Merge(const HistogramView& other) noexcept {
    if (!handle_ || !other.c_handle()) return Status(StatusCode::kInvalidArgument);
    return histo_add(mut_handle(), other.c_handle());
  }
 ...
 private:
  histo_t* mut_handle() noexcept {
    return const_cast<histo_t*>(handle_);
  }
};
