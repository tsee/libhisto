...
subtest 'Arithmetic operators and overloading' => sub {
    ...
    is $h_add->total_weight, within(6.0, 1e-9), '3 + 3 = 6';
    is $h_add->bin_content(1), within(2.0, 1e-9), 'bin 1 has 2.0';

    # Subtraction
    my $h_sub = $h_add - $h1;
    is $h_sub->total_weight, within(3.0, 1e-9), '6 - 3 = 3';

    # Scalar multiplication and division
    my $h_mul = $h1 * 2.0;
    is $h_mul->total_weight, within(6.0, 1e-9), 'scaled 3 * 2 = 6';

    my $h_div = $h_mul / 2.0;
    is $h_div->total_weight, within(3.0, 1e-9), 'divided 6 / 2 = 3';
...
subtest 'Distance and comparison metrics' => sub {
    ...
    is $ks, within(0.0, 1e-9), 'KS distance is 0 for identical histograms';

    my $w1 = $h1->wasserstein_distance($h2);
    is $w1, within(0.0, 1e-9), 'Wasserstein distance is 0 for identical histograms';

    my $bhat = $h1->bhattacharyya_distance($h2);
    is $bhat, within(0.0, 1e-9), 'Bhattacharyya distance is 0 for identical histograms';

    # Chi-square test
    my ($chi2, $ndf) = $h1->chi2_test($h2);
    is $chi2, within(0.0, 1e-9), 'chi2 is 0 for identical histograms';
    ok $ndf > 0, 'ndf > 0';
};
