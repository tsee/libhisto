...
- [x] **TEST-2**: Dedicated hermetic minimal C99 compiler test target (`make test-hermetic`) using clean `debian:bookworm-slim` with zero external interpreter dependencies.
- [x] **TEST-3**: Fuzzing coverage expansion across input parsers (`bpftrace`, `cli_opt`, `tui_command`), numerical engines (`kde`, `auto_bin`), and differential SIMD/roundtrip invariants.
